...
extension Parser {
  ...
  ///
  /// func-decl ::= fn <name>([<name> [internal-name]: <typename>,]*): <typename>
  /// <braced-expr-block>

  func parseFuncDecl(
    _ attributes: [DeclAccessKind],
    ...
    if case .deinitializer = kind {} else {
  ...
  func parseType() throws -> TypeRefExpr {
    ...
    while true {
      ...
      case .unknown(let char):
        ...
        consumeToken()
        return PointerTypeRefExpr(
          pointedTo: try parseType(),
          level: pointerLevel,
          sourceRange: range(start: startLoc)
        )
      case .leftParen:
        ...
        if case .arrow = peek() {
          ...
          return FuncTypeRefExpr(
            argNames: args,
            retName: ret,
            sourceRange: range(start: startLoc)
          )
        } else {
          return TupleTypeRefExpr(
            fieldNames: args,
            sourceRange: range(start: startLoc)
          )
        }
